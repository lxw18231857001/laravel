...
.explode {
  background:url({{asset('/resources/assets/admin/Images/menu_minus.gif')}}) no-repeat 0px 3px;
  font-weight:bold;
}
.collapse {
  background:url({{asset('/resources/assets/admin/Images/menu_plus.gif')}}) no-repeat 0px 3px;
  font-weight:bold;
}
.menu-item {
  background:url({{asset('/resources/assets/admin/Images/menu_arrow.gif')}}) no-repeat 0px 3px;
  font-weight:normal;
}
#help-title {
  font-size:14px;
    ...
    <p>
        <span style="float:right; padding:3px 5px;" >
            <a href="javascript:toggleCollapse();">
                <img id="toggleImg" src="{{asset('/resources/assets/admin/Images/menu_minus.gif')}}" width="9" height="9" border="0" alt="闭合" />
            </a>
        </span>
...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php foreach($pri_data as $v):?>
            @if($v->parent_id==0)
            <li class="explode" key="02_cat_and_goods" name="menu">
            {{ $v->pri_name }}

                <ul>
                    <?php foreach($pri_data as $v1):?>
                    @if($v1->parent_id == $v->id)
                    <li class="menu-item"><a href="{{ url($v1->module_name.'/'.$v1->controller_name.'/'.$v1->action_name) }}" target="main-frame">{{ $v1->pri_name }}</a></li>
                       @endif
                        <?php endforeach;?>
                </ul>
            </li>
...
<script language="JavaScript">
    ...
    function toggleCollapse() {
        ...
        collapse = !collapse;
        document.getElementById('toggleImg').src = collapse ? '{{asset('/resources/assets/admin/Images/menu_minus.gif')}}' : '{{asset('/resources/assets/admin/Images/menu_plus.gif')}}';
